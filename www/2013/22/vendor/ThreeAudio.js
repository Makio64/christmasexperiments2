...
FFT.prototype.inverse = function(real, imag, targetBuffer) {
  // Locally scope variables for speed up
  var bufferSize      = this.bufferSize,
      cosTable        = this.cosTable,
      sinTable        = this.sinTable,
  ...
  var buffer = targetBuffer || new Float32Array(bufferSize); // this should be reused instead
  for (i = 0; i < bufferSize; i++) {
...
ThreeAudio.Source = function (options) {
  ...
  this.processSize = options.processSize;
  this.generate = options.generate;
  this.generateSize = options.generateSize;
...
ThreeAudio.Source.prototype = {
  ...
  initElement: function (element) {
    var c = this.context = new (window.webkitAudioContext || window.AudioContext)();
  ...
  initAnalyzer: function () {
    ...
    if (this.process) {
      this.processor = c.createScriptProcessor(this.processSize);

      this.source.connect(this.processor);
      this.processor.connect(this.audible);
    ...
    if (this.generate) {
      this.generator = c.createScriptProcessor(this.generateSize, 0, 2);
      this.generator.connect(this.audible);
    }
  ...
  mic: function (callback) {
    ...
    try {
      var get = navigator.webkitGetUserMedia || navigator.mozGetUserMedia || navigator.getUserMedia;
      get.call(navigator, {
          audio: true
        }, function (stream) {
